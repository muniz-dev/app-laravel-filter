    ...
    <div class="w-full bg-white border-b border-gray-200 py-10">
        <div class="w-full px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-8">
                ...
                @if(!empty($name) || !empty($selectedCategories) || !empty($selectedBrands))
                    <button 
                        ...
                        >
                            <span class="text-sm font-medium">
                                @if(count($selectedCategories) > 0)
                                    {{ count($selectedCategories) }} selecionada(s)
                                @else
                        ...
                        >
                            <span class="text-sm font-medium">
                                @if(count($selectedBrands) > 0)
                                    {{ count($selectedBrands) }} selecionada(s)
                                @else
                                    Selecione marcas
                                @endif
                            </span>
                ...
                <div class="mt-8 pt-6 border-t-2 border-gray-200">
                    ...
                    <div class="flex flex-wrap gap-2.5">
                        ...
                        @if(!empty($selectedCategories))
                            <span class="inline-flex items-center px-4 py-2 bg-orange-100 text-orange-800 rounded-lg text-sm font-semibold border border-orange-300 shadow-sm">
                                {{ count($selectedCategories) }} categoria(s)
                            </span>
                        @endif
                        @if(!empty($selectedBrands))
                            <span class="inline-flex items-center px-4 py-2 bg-orange-100 text-orange-800 rounded-lg text-sm font-semibold border border-orange-300 shadow-sm">
                                {{ count($selectedBrands) }} marca(s)
                            </span>
                        @endif
                    </div>
                </div>
    ...
    <div class="w-full bg-white border-t border-gray-200 py-10">
        <div class="w-full px-4 sm:px-6 lg:px-8 mb-8">
            ...
            <p class="text-base text-gray-600">
                <span class="font-bold text-orange-600">{{ $products->total() }}</span> produto(s) disponível(is)
            </p>
        </div>
                                ...
                                <div class="pt-4 border-t-2 border-gray-100">
                                    <div class="flex items-baseline justify-between">
                                        ...
                                        <div class="text-right">
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm {{ $product->stock > 0 ? 'bg-green-100 text-green-800 border border-green-300' : 'bg-red-100 text-red-800 border border-red-300' }}">
                                                @if($product->stock > 0)
